...
plt.axis('off')
plt.title('Predicted Depth Map Masked')

plt.subplots_adjust(wspace=0.1, hspace=0.01)

# Create a single colorbar that applies to both depth images
cbar = plt.colorbar(pred_img, ax=plt.gcf().axes[1:], orientation='horizontal', fraction=0.02, pad=0.04)
cbar.set_label('Depth Value')  # Set the colorbar label

# Save and show the plot
plt.savefig('DEMO/Single_depth_demo/comparison.png', bbox_inches='tight')
plt.show()
